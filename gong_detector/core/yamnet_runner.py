...
class YAMNetGongDetector:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            print(f"Model loaded successfully. Total classes: {len(self.class_names or [])}")
            print(f"Gong class (index {self.gong_class_index}): {self._get_gong_class_name()}")
    ...
    def load_and_preprocess_audio(self, audio_path: str) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            print(f"Audio loaded: {len(waveform)} samples at {self.target_sample_rate}Hz")
    ...
    def _resample_if_needed(self, waveform: np.ndarray, current_rate: int) -> np.ndarray:
        ...
        resampled = tf.signal.resample(
            waveform_tensor,
            int(len(waveform) * self.target_sample_rate / current_rate)
        )
    ...
    def _calculate_hop_length(self, audio_duration: Optional[float], num_predictions: int) -> float:
