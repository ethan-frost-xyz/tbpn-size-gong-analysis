...
def plot_spectrograms(
    ...
    print("\n=== Audio Statistics ===")
    print(f"TBPN Gong: {tbpn_stats['duration']:.3f}s, Peak: {tbpn_stats['peak_amplitude']:.3f}")
    print(f"Reference Gong: {reference_stats['duration']:.3f}s, Peak: {reference_stats['peak_amplitude']:.3f}")
...
def main() -> None:
    ...
    parser.add_argument(
        "tbpn_path",
        help="Path to TBPN gong audio file (.wav)"
    )
    parser.add_argument(
        "reference_path", 
        help="Path to reference gong audio file (.wav)"
    )
