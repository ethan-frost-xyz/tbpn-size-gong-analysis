...
class TestLocalFileConversion:
    ...
    def test_convert_local_file_success(self, mock_exists: Mock, mock_convert: Mock) -> None:
    ...
    def test_cleanup_on_conversion_failure(
        ...
        mock_exists.side_effect = [False, True, True]  # URL, downloaded file, exists for cleanup
        mock_download.return_value = "temp_download.mp3"
...
class TestYouTubeDownload:
    ...
    @patch('os.listdir')
    @patch('os.path.exists')
    def test_download_audio_success(self, mock_exists: Mock, mock_listdir: Mock, mock_ydl_class: Mock) -> None:
    ...
    @patch('os.listdir')
    @patch('os.path.exists')
    def test_download_audio_file_not_found(self, mock_exists: Mock, mock_listdir: Mock, mock_ydl_class: Mock) -> None:
...
class TestAudioConversion:
    ...
    @patch('os.path.exists')
    @patch('os.makedirs')
    def test_convert_to_wav_success(self, mock_makedirs: Mock, mock_exists: Mock, mock_run: Mock) -> None:
    ...
    @patch('subprocess.run')
    @patch('os.path.exists')
    def test_convert_to_wav_ffmpeg_not_found(self, mock_exists: Mock, mock_run: Mock) -> None:
    ...
    @patch('subprocess.run')
    @patch('os.path.exists')
    def test_convert_to_wav_ffmpeg_error(self, mock_exists: Mock, mock_run: Mock) -> None:
    ...
    @patch('os.path.exists')
    @patch('os.makedirs')
    def test_convert_to_wav_creates_output_dir(self, mock_makedirs: Mock, mock_exists: Mock, mock_run: Mock) -> None:
        """Test that output directory is created if it doesn't exist."""
        mock_exists.side_effect = [True, False, True]  # Input exists, dir doesn't exist, output created
...
class TestAudioValidation:
    ...
    @patch('os.path.exists')
    def test_validate_audio_file_exists_with_audio_extension(self, mock_exists: Mock) -> None:
...
class TestAudioInfo:
    ...
    @patch('subprocess.run')
    @patch('os.path.exists')
    def test_get_audio_info_ffprobe_error(self, mock_exists: Mock, mock_run: Mock) -> None:
    ...
    @patch('subprocess.run')
    @patch('os.path.exists')
    def test_get_audio_info_no_audio_stream(self, mock_exists: Mock, mock_run: Mock) -> None:
...
class TestMainScript:
    ...
    def test_main_script_success(
        ...
        from gong_detector.core import convert_audio
    ...
    def test_audio_extensions_comprehensive(self) -> None:
        ...
        common_extensions = ['.wav', '.mp3', '.m4a', '.flac', '.ogg', '.aac', '.webm', '.mp4']
        
        with patch('os.path.exists', return_value=True):
            for ext in common_extensions:
                assert validate_audio_file(f"test{ext}"), f"Extension {ext} should be valid"
                assert validate_audio_file(f"test{ext.upper()}"), f"Extension {ext.upper()} should be valid" 